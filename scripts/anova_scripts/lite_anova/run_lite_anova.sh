...
N_QUERIES_VALUES=(8)
QPP_VALUES=("no")
FIRST_STEP_RANKER_VALUES=("both_fusion")
                        ...
                        for initial_retrieval_k_docs in "${INITIAL_RETRIEVAL_K_DOCS_VALUES[@]}"; do
                            ...
                            for reranker in "${RERANKER_VALUES[@]}"; do
                                ...
                                for context_words_limit in "${CONTEXT_WORDS_LIMIT_VALUES[@]}"; do
                                    ...
                                    output_dir="data/anova_result/${query_expansion_mode}_${n_queries}_${query_gen_prompt_level}_${qpp}_${initial_retrieval_k_docs}_${first_step_ranker}_${reranker}_${context_words_limit}_${rag_prompt_level}"
                                    
                                    # Skip if output directory exists and contains run files
                                    if [ -d "$output_dir" ] && [ -n "$(ls -A $output_dir/${reference_file}.run*.AnovaRAGLite.tsv 2>/dev/null)" ]; then
                                        echo "Skipping existing configuration: $output_dir"
                                        continue
                                    fi
                                    
                                    common_args="--llm_client $llm_client --system AnovaRAGLite --input $input --output-dir $output_dir --num-threads 20 --query_expansion_mode=$query_expansion_mode --n_queries=$n_queries --qpp=$qpp --initial_retrieval_k_docs=$initial_retrieval_k_docs --first_step_ranker $first_step_ranker --reranker $reranker --context_words_limit $context_words_limit --rag_prompt_level $rag_prompt_level --query_gen_prompt_level $query_gen_prompt_level"
                                    echo "Running with query_expansion_mode=$query_expansion_mode, n_queries=$n_queries, qpp=$qpp, first_step_ranker=$first_step_ranker, initial_retrieval_k_docs=$initial_retrieval_k_docs, reranker=$reranker, context_words_limit=$context_words_limit, query_gen_prompt_level=$query_gen_prompt_level, rag_prompt_level=$rag_prompt_level"
