...
def worker_process(model_id, gpu_id, request_queue, response_queue, status_queue, shutdown_event):
    ...
    try:
        ...
        if gpu_id >= 0 and torch.cuda.is_available():
            ...
            logger.info(
                f"Worker using GPU {gpu_id} (mapped to cuda:0): {torch.cuda.get_device_name(0)}")
            ...
            try:
                ...
                if request_type == REQUEST_TYPE_LOGITS:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error processing logits request: {str(e)}")
...
class BatchProcessor:
    ...
    def __init__(self, model_id: str, max_batch_size: int = 8, num_workers: int = None):
        ...
        logger.info(
            f"Initializing batch processor with {self.num_workers} workers")
        ...
        while self.response_handler_running:
            try:
                ...
                if not self.status_queue.empty():
                    ...
                    logger.info(
                        f"Model status update: {status_type} - {message}")
                    ...
                    if status_type == "ready":
                        ...
                        if self.workers_ready >= self.num_workers:
                            ...
                            logger.info(
                                "All workers are ready to serve requests")
    ...
    def shutdown(self):
        ...
        for i, process in enumerate(self.workers):
            ...
            if process.is_alive():
                logger.warning(
                    f"Worker {i} did not terminate gracefully, terminating")
                process.terminate()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default='tiiuae/falcon3-10b-instruct',
    ...
    async def health_check():
        ...
        workers_alive = all(worker.is_alive()
                            for worker in batch_processor.workers)
